...
OVERRIDES = {
    ...
    "practice-ao.example": (
        "NOT A PROSPECT - the practice was absorbed into a multi-state "
        "dermatology group. Port 443 accepts no connections (TCP connect times "
        "out at ~21s, in curl and in real Chrome alike); port 80 serves a 301 "
        "to a landing page on the acquirer's site, which renders 'Uh oh, page "
        "not found'. The broken redirect is real, but the prospect is now a "
        "location of a chain, not an independent local practice, and the "
        "acquirer's site 429s every automated request. Checked in Chrome "
        "2026-07-30."),
}
...
def audit(row):
    ...
    # Was: any year 1900-2015 anywhere in the body + the word "copyright"
    # anywhere. On the Phoenix run that fired on 9 of 19 sites and was wrong
    # every time - it matched CSS class names (.ast-footer-copyright), numeric
    # CSS values that look like years, FontAwesome's bundled license comment,
    # and footers whose year is written by new Date().getFullYear() or a
    # {{right_now.year}} template tag. Now it must find a year attached to a
    # copyright symbol, and only counts if nothing on the page renders the year
    # dynamically. Demoted out of the score: it is a render-check lead at best.
    stale = re.search(r"(?:copyright|©|&copy;)[^0-9<]{0,20}\b((?:19|20)\d\d)\b", body, re.I)
    dynamic = re.search(r"getFullYear|\{\{[^}]*year|\(\(\s*right_now\.year", body, re.I)
    if stale and not dynamic and int(stale.group(1)) < 2024:
        out.setdefault("needs_render_check", []).append(
            f"footer copyright reads {stale.group(1)}")
